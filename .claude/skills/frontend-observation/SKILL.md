...
差分の実体比較は `${CLAUDE_SKILL_DIR}/../figma2web/tools/visual-diff/visual_diff.py`（figma2web スキルに同梱、OpenCV ベース、領域分割＋テキスト除外）を使う。単純な RMS 比較で全面を見ない。Python は `.context/figma2web/.venv/bin/python` で実行（venv は figma2web の初回実行時に作られる）。図的に figma2web と同じ `.claude/skills/` に並ぶので `../figma2web/...` で必ず解決できる（project/personal/plugin いずれも）。
